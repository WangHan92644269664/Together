...
export default {
  ...
  methods: {
	  getAreas(){
		this.$utils.postUrl('GET','areas?page=1&per-page=2147483647').then(res=>{
			for(let item of res.data){
				if(item.area_depth===1){
					this.provinceDataList.push({
						'label':item.area_name,
						'value':item.area_id.slice(0,2)
					})
				}
				if(item.area_depth===2){
					this.cityData.push([
						{
							'label':item.area_name,
							'value':item.area_id.slice(0,4)
						}
					])
				}

				if(item.area_depth===3){
					this.areaData.push([
						{
							'label':item.area_name,
							'value':item.area_id
