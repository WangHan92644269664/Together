		...
		<view class="upload">
			<block v-for="(upload,index) in uploads" :key="index">
				<view class="uplode-file">
					...
					<text class="iconfont icon-guanbi clear-one-icon" v-if="types == 'image'"  @tap="delImage(index)"></text>
					<video v-if="types == 'video'" class="uploade-img" :src="upload" controls>
						<text class="iconfont icon-guanbi clear-one-icon"   @tap="delImage(index)"></text>
					</video>
				</view>
			</block>
					...
					case 'image': 
						uni.chooseImage({
							...
							success: (res) => {
								...
								this.$emit('successImage',this.uploadImages)
							},
							fail: (err) => {
								uni.showModal({
